...
network make_network(int const n)
{
    return (network){
        .n=n,
...
int resize_network(network * const net, int w, int h)
{
#ifdef GPU
    cuda_set_device(net->gpu_index);
    if (gpu_index >= 0) {
...
#ifdef GPU
    int const size = get_network_input_size(*net) * net->batch;
    if (gpu_index >= 0) {
...
{
    for (int i = net.n - 1; i >= 0; --i) {
        image const m = get_network_image_layer(net, i);
        if (m.h != 0) {
            return m;
        }
    }
...
void top_predictions(network net, int const k, int * const index)
{
    int const size = get_network_output_size(net);
    float * const out = get_network_output(net);
...
void copy_cudnn_descriptors(layer const src, layer * const dst)
{
#ifdef CUDNN
