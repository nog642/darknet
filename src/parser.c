...
void load_convolutional_weights(layer l, FILE* fp)
{
    ...
    int num = l.nweights;
    int read_bytes = fread(l.biases, sizeof(float), l.n, fp);
    ...
    // }
    // if (l.c == 3) {
    //     scal_cpu(num, 1. / 256, l.weights, 1);
    // }
    if (l.flipped) {
        transpose_matrix(l.weights, (l.c / l.groups) * l.size * l.size, l.n);
    }
    // if (l.binary) {
    //     binarize_weights(l.weights, l.n, (l.c/l.groups) * l.size * l.size, l.weights);
    // }
#ifdef GPU
    if (gpu_index >= 0) {
        push_convolutional_layer(l);
